...
def prepare_metadata(input_jsonl, output_jsonl, fields):
    ...
    with open(input_path, "r", encoding="utf-8") as f_in, open(output_path, "w", encoding="utf-8") as f_out:     #open 2 json files 
        for line in f_in:
            ...
            record = json.loads(line) # convert json line into a dictionaru

            filtered = {field: record.get(field) for field in fields} # extract requested field from record'
...
def main():
    ...
    parser.add_argument("-o", "--output", default="classification_data.jsonl", help="Path to filtered output JSONL.")
